    ...
    public sealed class RelativityImport : CompoundStep<Unit>
    {
        ...
        protected override async Task<Result<Unit, IError>> Run(IStateMonad stateMonad, CancellationToken cancellation)
        {
            ...
            var arguments = await TryGetArguments(settingsResult.Value)
                .Bind(settingsArguments => TryGetArguments(stateMonad, settingsArguments, this, cancellation));
        ...
        private static async Task<Result<Maybe<KeyValuePair<string, string>>, IError>> TryMakeArgument<T>(
            IStep<T>? step,
            bool required,
            IStateMonad stateMonad,
            string flag,
            string argumentName,
            Func<T, string> map, IStep callingStep, CancellationToken cancellation)
        {
            if (step == null)
            {
                if (required)
        ...
        private async Task<Result<List<string>, IError>> TryGetArguments(
            IStateMonad stateMonad,
            IEnumerable<KeyValuePair<string, string>> otherArguments, IStep callingStep, CancellationToken cancellation)
        {
            var argsResults = new[]
            {
                await TryMakeArgument(FilePath, true, stateMonad, "f", nameof(FilePath), x => x.GetString(), callingStep,
                    cancellation),
                await TryMakeArgument(WorkspaceId, true, stateMonad, "c", nameof(WorkspaceId), x => x.ToString(),
                    callingStep, cancellation),
                await TryMakeArgument(FileImportType, true, stateMonad, "m", nameof(FileImportType),
                    GetFileImportTypeString, callingStep, cancellation),
                await TryMakeArgument(SettingsFilePath, true, stateMonad, "k", nameof(SettingsFilePath), x => x.GetString(),
                    callingStep, cancellation),
                await TryMakeArgument(StartLineNumber, false, stateMonad, "s", nameof(StartLineNumber),
                    x => x.ToString(), callingStep, cancellation),
                await TryMakeArgument(DestinationFolder, false, stateMonad, "d", nameof(DestinationFolder),
                    x => x.ToString(), callingStep, cancellation),
                await TryMakeArgument(LoadFileEncoding, false, stateMonad, "e", nameof(LoadFileEncoding),
                    x => x.ToString(), callingStep, cancellation),
                await TryMakeArgument(FullTextFileEncoding, false, stateMonad, "x", nameof(FullTextFileEncoding),
                    x => x.ToString(), callingStep, cancellation),
            };
