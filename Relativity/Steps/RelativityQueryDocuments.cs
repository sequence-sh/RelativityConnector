...
namespace Reductech.EDR.Connectors.Relativity.Steps
{
    ...
    /// </summary>
    public sealed class RelativityQueryDocuments : RelativityApiRequest<(int workspaceId, QueryRequest request, int
        indexOfFirst, int lengthOfResults), IObjectManager, QueryResult, Array<Entity>>
    {
        /// <inheritdoc />
        public override IStepFactory StepFactory { get; } = new SimpleStepFactory<RelativityQueryDocuments, Array<Entity>>();

        /// <inheritdoc />
        public override Result<Array<Entity>, IErrorBuilder> ConvertOutput(
            QueryResult serviceOutput)
        {
            return TryConvertToEntityArray(serviceOutput.Objects);
        }

        /// <inheritdoc />
        public override Task<QueryResult> SendRequest(IStateMonad stateMonad, IObjectManager service,
            (int workspaceId, QueryRequest request, int indexOfFirst, int lengthOfResults) requestObject,
        ...
        public override async
            Task<Result<(int workspaceId, QueryRequest request, int indexOfFirst, int lengthOfResults), IError>>
            TryCreateRequest(IStateMonad stateMonad, CancellationToken cancellation)
        {
            ...
            if (data.IsFailure)
                return data
                    .ConvertFailure<(int workspaceId, QueryRequest request, int indexOfFirst, int lengthOfResults)>();

            var (workspaceArtifactId, condition, artifactIds, start, length, artifactTypeId, sortArtifactId,
            ...
            if (sortArtifactId.HasValue)
            {
                sorts.Add(new Sort()
                {
                    Direction = sortDirection, FieldIdentifier = new FieldRef() { ArtifactID = sortArtifactId.Value }
                });
            }
        ...
        public IStep<SortEnum> SortDirection { get; set; } = new EnumConstant<SortEnum>(SortEnum.Ascending);
