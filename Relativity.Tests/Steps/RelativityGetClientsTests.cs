...
namespace Reductech.EDR.Connectors.Relativity.Tests.Steps
{
    public partial class RelativityGetClientsTests : StepTestBase<RelativityGetClients, Array<Entity>>
    {
        ...
        protected override IEnumerable<StepCase> StepCases
        {
            get
            {
                yield return new StepCase("Get Clients",
                        TestHelpers.LogAllEntities(
                            new RelativityGetClients()
                        ), Unit.Default, "(Name: \"Client 1\" ArtifactID: 1 Guids: \"\")",
                        "(Name: \"Client 2\" ArtifactID: 2 Guids: \"\")"
                    )
                    .WithTestRelativitySettings()
                    .WithService(new MockSetup<IMatterManager, List<DisplayableObjectIdentifier>>(
